...
@app.route('/')
def home(): return "Manual Approval Bot is Running!"
...
def start_handler(message):
    ...
    if match:
        ...
        if u_data and u_data.get('expiry', 0) > datetime.now().timestamp():
            ...
            bot.send_message(uid, "🔒 **Prime Membership Required!**\n\nNiche se plan select karein aur payment screenshot bhein.", reply_markup=markup)
            return

    if uid == ADMIN_ID:
        bot.send_message(uid, "👑 **ADMIN PANEL**\n/short - Create Link\n/stats - Check Users")
    else:
        bot.send_message(uid, "👋 Welcome! Movie links dekhne ke liye unpar click karein.")
...
def handle_pay(call):
    ...
    bot.send_photo(call.message.chat.id, qr_api, caption=f"💰 **Amount: ₹{price}**\n\n1. Payment karein.\n2. Iska **SCREENSHOT** yahan bhein.\n\nAdmin turant check karke link de denge.")
